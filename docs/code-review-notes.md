...
- **And then the same question one level up: what _else_ satisfies the selector?** Two premises
  were still unasserted. `main[aria-busy="true"]` is rendered by five route skeletons, not one, so a
  boundary added at `app/(app)/` would keep this test green with `w/[workspaceId]/loading.tsx`
  deleted; it watches `main[data-workspace-skeleton]` now, which only that file renders. And
  extracting the observer into `e2e/watch-for.ts` moved the seeding into the helper, which quietly
  cost `navigation.spec.ts` the `false` start it used to hard-code — it asserts the bar is down
  before watching for it now, the same premise this test asserts about the skeleton. Narrowing a
  selector is not the same as naming the one thing you mean.
...
- **What the measurement said, rather than what seemed likely**: the LCP element is the screenshot
  itself. On a 412px-wide phone viewport its top sits at 676px, inside an 823px fold, so it is a
  paint candidate wherever the section goes — moving it below the feature cards changed nothing,
  and neither did `priority`, because the cost is render rather than fetch (1.3s render delay
  against 88ms of transfer).

- **What was tried and rejected**: the section above the cards and below them, `priority` on both
  images, and a narrower rendered width. All measured, all 94. The two points are what a 1.2-megapixel
  raster costs to decode and paint under a 4× CPU throttle, and no arrangement of it avoids that.

- **Shipped with the number recorded rather than tuned away.** The README's table is filled from
  deployed runs, where the last landing measurement was 98 on Vercel's CDN; this one is a local
  `next start` with the image optimizer warming on demand, so it is a lower bound rather than the
  figure that goes in the table. The deployed page is re-measured after this ships, and if it lands
  under 95 the fix has a real number behind it instead of a guess.

- **Lesson**: **an image is not a below-the-fold image on a phone.** A fold is 823px tall and a hero
  is not, so "put it lower" is a desktop intuition — the only things that move an LCP of this kind
  are fewer pixels or no picture.

## A type that only existed after a build, 20 September 2026

- **Issue**: importing a screenshot for `next/image` passed lint and typecheck here and failed both
  in CI — "Unsafe assignment of an error typed value" on the two imports. Nothing about the code
  differed.

- **Cause**: `next-env.d.ts` declares `*.png`, and it is **generated by `next build` and gitignored**.
  This machine had one from an earlier build; a clean checkout does not, and CI runs lint and
  typecheck before it builds. The import was therefore untyped exactly where it mattered and typed
  everywhere it did not.

- **Fix**: `types/images.d.ts` declares the module against `StaticImageData`, so the type does not
  depend on a build having run. Reproduced properly before and after: with `next-env.d.ts` and
  `.next/` moved aside, lint reports the same two errors and typecheck adds "Cannot find module", and
  with the declaration both pass.

- **Then the fix failed the same way, for a different reason.** The declaration used an inline
  `import()` type annotation, which this repository's lint forbids — unnoticed because what was
  linted afterwards was the component, not the file being added. It is a top-level `import type`
  inside the module block now, and what settled it was `eslint .` in a worktree checked out fresh
  from the pushed commit, with no generated types in it.
