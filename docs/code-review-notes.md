...
## A comment asserting three properties inherit, when one of them does

- **Issue**: one declaration styled the app's scrollbars:

  ```css
  html {
    /* Both properties inherit, so one declaration covers every scroll container. */
  ...
  The commit that added it said it replaced native scrollbars with thin themed ones. It replaced
  exactly one: the document's. The composer textarea and the source panel kept full-width native
  scrollbars for two milestones.

- **Cause**: of the three, only `scrollbar-color` inherits. `scrollbar-width` and
  `scrollbar-gutter` are per-element, so nothing below `<html>` received either.

  **What hid it is that the inheriting property is the visible one.** The nested scrollers did
  change — they picked up the themed thumb color — so the declaration visibly did something
  everywhere it was looked at. Confirming it required `getComputedStyle` on a nested scroller
  rather than a glance: it reports `scrollbar-width: auto`, not `thin`.

- **Second defect, from the same line**: `scrollbar-gutter: stable` reserves space outside the
  content box that no element can paint into, so the header's full-bleed border stopped 10px
  short of the window edge — 1270px in a 1280px window — on every page.

- **Fix**: ADR 020. Native scrollbars are hidden app-wide with `scrollbar-width: none` on the
  universal selector, and an overlay thumb is drawn over the content instead. `e2e/scrollbars.spec.ts`
  asserts the header and footer right edges equal `window.innerWidth`.

- **Lesson**: **a comment claiming a mechanism is a claim, and it was reviewed as a fact.** "Both
  properties inherit" was written into the file, read on review, and believed — including by the
  reviewer who wrote it. It is also the kind of claim that costs one line to check.

  The narrower version: **CSS inheritance is per property and there is no pattern to it.** These
  three sit in the same spec section, are set on the same element, and split two-to-one. Anything
  relying on inheritance to reach descendants needs the computed value read on a descendant, which
  is the only place the answer actually lives.

  This is the same failure as the regression test in Milestone 3 that could not have failed —
  a check written from what the author expected the platform to do rather than from what it was
  observed doing.
