...
- **The remaining cost is markers on refusals.** Over six runs, 2 of the golden five and 9 to 13 of
  the uncovered fifteen carry one per run, against the prompt's own rule. Halving that rate needs
  about 24 leaked questions per variant to detect, and the two sets leak 20. Stripping markers from
  a reply that opens with a refusal would be the structural fix, and it is in the backlog.
