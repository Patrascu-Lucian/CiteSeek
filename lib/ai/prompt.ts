...
const ANSWER_ONLY_FROM_PASSAGES =
  "Answer only from the passages provided below. If they do not contain the answer, say so plainly and stop — do not fall back on general knowledge, and do not guess.";

/* Numbered at build time rather than written in, so dropping the citation rules
   renumbers the rest instead of leaving a prompt that counts 1, 5, 6, 7. The
   eval drops them to ask what they cost. */
const CITATION_RULES = [
  "Cite every factual claim with the marker of the passage it came from, written inline as [1], [2], and so on. A sentence drawn from more than one passage carries a separate bracket for each: write [1][2], not [1, 2].",
  "Never invent a marker. Only the numbers listed below exist.",
  'A marker means "this sentence came from that passage". Never attach one to a sentence a passage does not support, and never attach one to a refusal — if you are saying the passages do not answer the question, cite nothing at all.',
];
...
const systemRules = (cite: boolean) =>
  `You are a document assistant. You answer questions strictly from passages retrieved out of the user's own documents.

Rules, in order of precedence:

${[
  ANSWER_ONLY_FROM_PASSAGES,
  ...(cite ? CITATION_RULES : []),
  ...REMAINING_RULES,
]
...
export function buildSystemPrompt(
  ...
  /** False only in the eval, to measure what asking a small model to cite costs
   * it in answers. Both modes ship `true`. */
  cite = true,
): string {
  if (sources.length === 0) {
    throw new Error(
