...
//
// Thrown rather than defaulted: with the wrong secret every negative test below
// passes for the wrong reason, and only the positive one notices. `@auth/core`
// keeps `secret` as an array, so `AUTH_SECRET_1..3` rotation would need the
// first entry here rather than a join.
const secret = () => {
